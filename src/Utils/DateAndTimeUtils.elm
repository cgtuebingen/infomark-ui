...
initTime : TP.Time
initTime =
    pickerTimeFromPosix Time.utc <| Time.millisToPosix 0
...
pickerTimeToMillis zone time =
    let
        ...
        offsetInSeconds =
            timeZoneToUtcOffsetMinutes zone * 60
    in
...
addPickerTimeToPosix posix zone pickerTime =
    Time.posixToMillis posix
        + pickerTimeToMillis zone pickerTime
        |> Time.millisToPosix
...
splitPosixInDateTimeAndOffset zone posix =
    let
        ...
        offsetParts =
            timeZoneToUtcOffsetMinutes zone |> offsetToParts
    in
...
timeZoneToUtcOffsetMinutes zone =
    let
        ...
        hour =
            Time.toHour zone <| Time.millisToPosix oneDay

        minute =
            Time.toMinute zone <| Time.millisToPosix oneDay

        day =
            Time.toDay zone <| Time.millisToPosix oneDay

        hourUtc =
            Time.toHour Time.utc <| Time.millisToPosix oneDay

        minuteUtc =
            Time.toMinute Time.utc <| Time.millisToPosix oneDay

        dayUtc =
            Time.toDay Time.utc <| Time.millisToPosix oneDay
...
partsToMinutes parts =
    utcOffsetMinutes parts.multiplier parts.hours parts.minutes
    ...
    case ( parts.hours, parts.minutes ) of
        ...
        ( _, _ ) ->
            let
                ...
                hour =
                    String.padLeft 2 '0' <| String.fromInt parts.hours

                minute =
                    String.padLeft 2 '0' <| String.fromInt parts.minutes
            in
            sign ++ hour ++ ":" ++ minute
...
utcOffsetLabelsList =
    List.map partsToLabel utcOffsetsPartsList


utcOffsetMinutesList : List Int
utcOffsetMinutesList =
    List.map partsToMinutes utcOffsetsPartsList
